...
create_maze(Width, High) ->
    ...
    Maze1 = maze_util:rem_wall({1, 1}, Maze),
    Heap = [{1, 1}],
...
gen_maze(Maze, [{X, Y} = Grid | T] = Heap, Width, High) ->
    Directions = maze_util:get_directions(Grid, 2, Width, High),
    case choice_wall(Directions, Maze) of
        none ->
            gen_maze(Maze, T, Width, High);
        {NX, NY} = ChoiceGrid ->
            Maze1 = maze_util:rem_wall(ChoiceGrid, Maze),
            WallGrid = {NX + max(min(X - NX, 1), -1), NY + max(min(Y - NY, 1), -1)},
            Maze2 = maze_util:rem_wall(WallGrid, Maze1),
